...
def astar(grid, obstacle_list):
    
    goal_x = grid.goal_x
    goal_y = grid.goal_y
    unvisited_nodes = dict()
    visited_nodes = dict()
    heuristic = utils.distance_calc(grid.start_x, grid.start_y, goal_x, goal_y)
    current_node = utils.node(grid.start_x, grid.start_y, 0, heuristic, -1)
        ...
        while current_node.x != goal_x or current_node.y != goal_y:
            ...
            for i in np.arange(-grid.spacing, grid.spacing + grid.spacing, grid.spacing):
                for j in np.arange(-grid.spacing, grid.spacing + grid.spacing, grid.spacing):
                    ...
                    heuristic = utils.distance_calc(temp_x, temp_y, goal_x, goal_y)
                    temp_node = utils.node(temp_x, temp_y, cost, heuristic, current_index)
                    ...
                    if main_check == True:
                        if unvisited_nodes.get(temp_index, False) == True  and temp_node.total_cost < unvisited_nodes[temp_index].total_cost:
                            unvisited_nodes[temp_index].total_cost = temp_node.total_cost 
...
#    plt.plot(path_x, path_y, label="A*")
